...
func ValidateBook(v *validator.Validator, book *Book) {
	...
	v.Check(book.Year <= (time.Now().Year()), "year", "must not be in the future")

	v.Check(book.Author != "", "author", "must be provided")
	v.Check(len(book.Title) <= 20, "author", "must not be more than 20 bytes long")
...
func (b BookModel) Insert(book *Book) error {
	query := `INSERT INTO books (title, year, author, genres, price, description)
			  VALUES ($1, $2, $3, $4, $5, $6)
...
func (b BookModel) GetById(id int64) (*Book, error) {
	...
	query := `
		SELECT id, created_at, title, year, author, genres, price, description, version
		FROM books
		WHERE id = $1`
...
func (b BookModel) Update(book *Book) error {
	query := `
		UPDATE books
		SET title = $1, year = $2, author = $3, genres = $4, price = $5, description = $6, version = version + 1
...
func (b BookModel) GetAll(title string, genres []string, filters Filters) ([]*Book, Metadata, error) {
	query := fmt.Sprintf(`
		SELECT count(*) OVER(), id, created_at, title, year, author, genres, price, description, version
		FROM books
